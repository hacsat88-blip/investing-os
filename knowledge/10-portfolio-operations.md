# 10 保有更新・相互編集・ダッシュボード v4.4

2026-09-23改訂：会話AIの経路、actorの記載、米国株・米国ETFの保持規則を追加。他は v4.3 から変更なし。
2026-09-24追補：実行AI名の記載先と、米国上場の判定方法を明記。

## 更新手順
1. data/current.jsonの版を読み、同版のCSV全12種を検証する。app/store.py read を利用する。会話AIはこの手順を実行できないため、アプリの「AI用データ出力」を読んで同じ検証を行う（70-health-and-agents.md）。
...
6. 提案JSON（version、tables、reason）を作り、アプリに読み込む。ユーザーも編集できる。差分確認→保存→再読とバックアップ検査の完了を確認する。提案JSONのreasonに実行AI（例 ai:claude-code）を記す。保存履歴のactor欄はAIのまま。
...
米国株・米国ETFは、ティッカーを文字列で保持し、数量は端株を丸めない。評価は数量×USD価格×同時点FX。円建て取得原価（costBasisJpy）が無い行は損益空欄（取得単価×現在FXで補完しない）。
当面、currency=USDの保有を米国上場とみなし、保有台帳に取引所の列は持たない。取引所はquotes.csvのnoteにTradingViewシンボル（例 NYSE:LMT）で記す。米国以外の外貨建て商品を登録する時は、市場列の追加を別途検討する。
