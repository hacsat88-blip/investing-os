...
## スクリーニング境界
`scr/` の順位は「資金回転が大きい順」であり、投資品質、期待リターン、成功確率、買い順位ではない。全銘柄カバレッジを検証できない場合は「各市場TOP」と呼ばず「取得済み範囲内順位」とする。回転率の高低だけで `PROPOSED` の売買案、`APPROVED`、`EXECUTED`、CSV_SYNCEDを生成しない。ランキング後の財務・材料・リスク確認と投資評価は独立した工程とし、欠損・競合・陳腐化はUNKNOWNのまま残す。
