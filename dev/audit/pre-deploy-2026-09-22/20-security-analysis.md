...
東証市場横断の候補抽出は25-screening.mdの `scr/` を用いる。先に回転率を同一式・同一基準日で順位化し、その後に本書の企業分析を重ねる。順位を投資評価へ転用せず、上位候補ごとに売上、営業利益、営業CF、ROE、自己資本比率、赤字、希薄化、信用需給、材料依存、直近30暦日のTDnet/IR/ニュースを確認する。財務期間の違い、推定値、未取得値を表で混ぜない。
