...
NUMBERS = {'quantity','avgCost','costBasisJpy','price','fx','marketValueJpy','pnlJpy','weight','pts','ptsVolume','close','stars','impact','uncertainty','urgency','effort','additionalCashJpy','feesJpy','taxJpy','targetValueJpy'}
SIGNED = {'pnlJpy'}
...
class Store:
    ...
    def read_revision(self, version):
        ...
        for table in SCHEMAS:
            if table in EXTRA_SCHEMAS and table not in meta['hashes'] and meta['schemaVersion']=='4.2':
                tables[table]=[]
                continue
    ...
    def commit(self, tables, version, actor, reason):
        ...
        with self.lock():
            ...
            meta={'version':vid,'parent':version,'savedAt':now(),'actor':actor,'reason':reason,'hashes':hashes,'changes':changes,'schemaVersion':'4.3'}
